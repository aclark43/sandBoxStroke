function [thresh, par, threshB, xi, yi, chiSq] = psyfitCrowding(lvl, hits, varargin)
threshB = [];
xi = [];
yi = [];
chiSq = [];
...
switch distType
    ...
    case 'Weibull'
        if isLog
            % the 0.632 quantile of the weibull is approx alpha
            [~, idx] = min(abs(.632 - k./n));
            startPt(1) = log(lvl(idx));
...
% for bootstrapping the threshold.
    function [thresh, par] = findParams(k, canExtrapolate)
        ...
        lklF = @(par) -1 * likelihood(k, n, psyF(x, par), par, limits);
...
%     figure(plotHandle);
    ...
    if ~ isnan(thresh)
        ...
        yi = psyF(xi,par);
        plot(xi,yi, 'LineWidth',5,'Color',[0 0.5 0.5] );%%%Change width of line AMC
        hold on
    end
    ...
    for ii=1:length(x)%%%%%%%%%%%%
        if n(ii) >=1 % 10
            plot( x(ii) , k(ii) / n(ii) ,...
                'o','MarkerSize', 6,'Color',...
                [.3 .8 .7],'MarkerFaceColor',[.3 .8 .7],...
                 'LineWidth',ceil(n(ii)/10));
%                 'LineWidth',(n(ii)/20));
%                
             hold on
        end
...
%     if isLog
%         set( gca, 'xscale','linear');%%%%%%%%%%CHANGED TO LINEAR FIT AMC
%     end
% s
%     %SETS NUMBERS INSIDE DATA POINTS AMC
%     text(x , k ./ n, ...
%         num2str(n'),'Color',[0 0 0],...
%         'HorizontalAlignment','Center','FontWeight','bold');
    ...
    if ~ isnan(thresh)
%%%Commented out 12/18/2018 to eliminate extra text on graph AMC
        chiSq = (round(chisq*100)/100);
        text(txtPosX , yLo +.30 * yRg,...
            strcat('p({\chi^2}) = ',num2str(round(chisq*100)/100)),...
            'FontSize',fontsize,'Color',[.3 .3 .5]);
         text(txtPosX , yLo +.50 * yRg,...
            sprintf('g = %s',num2str(g)),...
            'FontSize',fontsize,'Color',[.5 .5 .5]);
