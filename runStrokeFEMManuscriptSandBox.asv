...
%% Params
reprocess = 1; %reload in all the pprtrials
...
for ii = 1:length(subjectsAll) % Link Specific IDs with Huxlin data
...
%% Load in pptrial Data
if reprocess == 1
    for i = 1:length(subjectsAll)
        
        filepath = sprintf...
            ('X:/Ashley/OLD_PreNewEyeris/HUXDataDDPI/ProcessedPPTrials/%s/Uncrowded_Unstabilized_0ecc', ...
            subjectsAll{i});
    ...
    for ii = 1:length(subjectsAll)
        validMS = [];
        [tracesMS, counterMS, validMS, emMS] = countingTrials(pptrialsLoaded{ii});
        clean_pptrials_Task{ii} = pptrialsLoaded{ii}(~validMS.fixation);
        clean_pptrials_Fixation{ii} = pptrialsLoaded{ii}(validMS.fixation);
        tossingInfo{ii} = validMS;
    end
    
    %% build table of all data from cleaned pptrials
    
    %Drift Sections Only
    counter = 1;
    for t = 1 %Not doing fixations because no fixations are drift only
        tableInfo(counter).Task = t;
        if t == 1
            roi{1} = 'TimeTargetON';
            roi{2} = 'TimeTargetOFF';
            dataType = clean_pptrials_Task;
        elseif t == 2 %Fixation
            roi{1} = 'TimeFixationON';
            roi{2} = 'TimeFixationOFF';
            dataType = clean_pptrials_Fixation;
        end
        for ii = 1:length(subjectsAll)
            for pp = 1:length(dataType{ii})
                if tossingInfo{ii}.d(pp) == 1
                    
                    tableInfo(counter).Subject = ii;
                    tableInfo(counter).TrialNum = dataType{ii}{pp}.id;
                    tableInfo(counter).Control = control(ii);
                    tableInfo(counter).Task = t;
                    tableInfo(counter).Performance = dataType{ii}{pp}.Correct;
                    if t == 1
                        tableInfo(counter).Size = double(dataType{ii}{pp}.TargetStrokewidth/2)*...
                            dataType{ii}{pp}.pixelAngle;
                    else
                        tableInfo(counter).Size = 16;
                    end
                    startTime = ceil(dataType{ii}{pp}.(roi{1})/(1000/341));
                    endTime = min([floor(dataType{ii}{pp}.(roi{2})/(1000/341)) length(dataType{ii}{pp}.x.position)]);
                    
                    
                    tx = dataType{ii}{pp}.x.position(startTime:endTime) + ...
                        dataType{ii}{pp}.pixelAngle * dataType{ii}{pp}.xoffset;
                    ty = dataType{ii}{pp}.y.position(startTime:endTime) + ...
                        dataType{ii}{pp}.pixelAngle * dataType{ii}{pp}.yoffset;
                    
                    idxNoNans = find(~isnan(tx) &...
                        ~isnan(ty) & tx < 200 & tx > -200 & ty < 200 & ty > -200);
                    tableInfo(counter).x = tx(idxNoNans);
                    tableInfo(counter).y = ty(idxNoNans);
                    
                    tableInfo(counter).xPlusOutTarget = dataType{ii}{pp}.x.position+ ...
                        dataType{ii}{pp}.pixelAngle * dataType{ii}{pp}.xoffset; %Time before and after target pres
                    tableInfo(counter).yPlusOutTarget = dataType{ii}{pp}.y.position+ ...
                        dataType{ii}{pp}.pixelAngle * dataType{ii}{pp}.yoffset;
                    ...
                    for d = 1:max([1 length(dataType{ii}{pp}.fixations)])
                        if isempty(tableInfo(counter).x)
                            tableInfo(counter).instSpX{d}= NaN;
                            tableInfo(counter).instSpY{d}= NaN;
                            tableInfo(counter).mn_speed(d)= NaN;
                            tableInfo(counter).driftAngle{d}= NaN;
                            tableInfo(counter).curvature(d)= NaN;
                            tableInfo(counter).varx(d)= NaN;
                            tableInfo(counter).vary(d)= NaN;
                            tableInfo(counter).bcea(d)= NaN;
                            tableInfo(counter).span(d)= NaN;
                            tableInfo(counter).amplitude(d)= NaN;
                            tableInfo(counter).prlDistance(d)= NaN;
                            tableInfo(counter).prlDistanceX(d)= NaN;
                            tableInfo(counter).prlDistanceY(d) = NaN;
                            continue
                        end
                        [~,  tableInfo(counter).instSpX{d},...
                            tableInfo(counter).instSpY{d},...
                            tableInfo(counter).mn_speed(d),...
                            tableInfo(counter).driftAngle{d},...
                            tableInfo(counter).curvature(d),...
                            tableInfo(counter).varx(d),...
                            tableInfo(counter).vary(d),...
                            tableInfo(counter).bcea(d),...
                            tableInfo(counter).span(d), ...
                            tableInfo(counter).amplitude(d), ...
                            tableInfo(counter).prlDistance(d),...
                            tableInfo(counter).prlDistanceX(d), ...
                            tableInfo(counter).prlDistanceY(d)] = ...
                            getDriftChar(tableInfo(counter).x, tableInfo(counter).y, 41, 1, 250, 341);
                        tableInfo(counter).avDirection{d} = rad2deg(circ_mean(cell2mat(tableInfo(counter).driftAngle)));
                        
                        tempT.x = tableInfo(counter).x;
                        tempT.y = tableInfo(counter).y;
                        
                        [~,tableInfo(counter).velocity(d), ...
                            tableInfo(counter).velocityX{d},...
                            tableInfo(counter).velocityY{d}] =...
                            CalculateDriftVelocity(tempT,[1 55]);
                        
                        [~,~,tableInfo(counter).dCoefDsq(d), ~, ~, ...
                            tableInfo(counter).SingleSegmentDsq{d},~,~] = ...
                            CalculateDiffusionCoef(330, struct('x',tempT.x, 'y', tempT.y));
                    end
    ...
    %MS Sections and corresponding drift
    for t = 1
        tableInfo(counter).Task = t;
        roi{1} = 'TimeTargetON';
        roi{2} = 'TimeTargetOFF';
        ...
        for ii = 1:length(subjectsAll)
            for pp = 1:length(dataType{ii})
                %                 if tossingInfo{ii}.dms(pp) == 1 || tossingInfo{ii}.fixation(pp)
                if isempty(dataType{ii}{pp}.x.position)
                    continue
                end
                tableInfo(counter).Subject = ii;
                tableInfo(counter).TrialNum = dataType{ii}{pp}.id;
                ...
                tableInfo(counter).SamplingRate = 341;
                tableInfo(counter).DriftOnly = 0;
                tableInfo(counter).MS = 1;
                tableInfo(counter).msStartIdx = dataType{ii}{pp}.microsaccades.start;
                tableInfo(counter).msDurationIdx = dataType{ii}{pp}.microsaccades.duration;
                tableInfo(counter).msEndIdx = dataType{ii}{pp}.microsaccades.start + ...
                    dataType{ii}{pp}.microsaccades.duration;
                
                if ~isempty(tableInfo(counter).msStartIdx)
                    if isempty(dataType{ii}{pp}.microsaccades.amplitude)
                        if sum(tableInfo(counter).msEndIdx <= 0) >= 1 ||...
                                sum(tableInfo(counter).msStartIdx <= 0) >= 1
                            
                            idx = find(tableInfo(counter).msEndIdx <= 0);
                            tableInfo(counter).msStartIdx(idx) = 1;
                            tableInfo(counter).msEndIdx(idx) = 1;
                        end
                        if sum(tableInfo(counter).msEndIdx > ...
                                length(tableInfo(counter).xPlusOutTarget)) > 0 ||...
                                sum(tableInfo(counter).msStartIdx > ...
                                length(tableInfo(counter).xPlusOutTarget))
                            idx = find(tableInfo(counter).msEndIdx > length(tableInfo(counter).xPlusOutTarget));
                            tableInfo(counter).msEndIdx(idx) = length(tableInfo(counter).xPlusOutTarget);
                            tableInfo(counter).msStartIdx(idx) = length(tableInfo(counter).xPlusOutTarget);

                        end
                        tableInfo(counter).msAmplitude = eucDist(...
                            tableInfo(counter).xPlusOutTarget(tableInfo(counter).msStartIdx)...
                            -tableInfo(counter).xPlusOutTarget(tableInfo(counter).msEndIdx), ...
                            tableInfo(counter).yPlusOutTarget(tableInfo(counter).msStartIdx)...
                            -tableInfo(counter).yPlusOutTarget(tableInfo(counter).msEndIdx));
                        
                        tableInfo(counter).msAngle = atan2d(...
                            tableInfo(counter).xPlusOutTarget(tableInfo(counter).msStartIdx)...
                            -tableInfo(counter).xPlusOutTarget(tableInfo(counter).msEndIdx), ...
                            tableInfo(counter).yPlusOutTarget(tableInfo(counter).msStartIdx)...
                            -tableInfo(counter).yPlusOutTarget(tableInfo(counter).msEndIdx));
                        
                        temp = ones(1,length(dataType{ii}{pp}.microsaccades.amplitude));
                        for m = 1:length(temp)
                            if dataType{ii}{pp}.microsaccades.start(m) < startTime(m) ||...
                                    dataType{ii}{pp}.microsaccades.start(m) > endTime(m)
                                tableInfo(counter).duringTrial(m) = 0;
                            else
                                tableInfo(counter).duringTrial(m) =  1;
                            end
                        end
                    else
                        tableInfo(counter).msAmplitude = dataType{ii}{pp}.microsaccades.amplitude;
                        tableInfo(counter).msAngle = dataType{ii}{pp}.microsaccades.angle;
                    end
                    if t == 1
                        tableInfo(counter).msRate = (length(tableInfo(counter).msStartIdx)) * 2; %Because the stim time is 500 ms
                    elseif t == 2
                        tableInfo(counter).msRate = (length(tableInfo(counter).msStartIdx)) / 5; %Because the stim time is 5s
                    end
                    tableInfo(counter).MS = 1;
                end
                %                     if t == 1
    ...
    for t = 2
        ...
        for ii = 1:length(subjectsAll)
            for pp = 1:length(dataType{ii})
                ...
                startTime = ceil(dataType{ii}{pp}.(roi{1})/(1000/341));
                endTime = min([floor(dataType{ii}{pp}.(roi{2})/(1000/341)) ...
                    length(dataType{ii}{pp}.x.position)]);
                ...
                idxNoNans = find(~isnan(tx) &...
                    ~isnan(ty) & tx < 6*60 & tx > -6*60 & ty < 6*60 & ty > -6*60);
                
                tableInfo(counter).x = tx(idxNoNans);
                tableInfo(counter).y = ty(idxNoNans);
                
                if length(tableInfo(counter).x) < floor(500/(1000/341))
                    continue;
                end
                tableInfo(counter).x500 = tableInfo(counter).x(1:floor(500/(1000/341)));
                tableInfo(counter).y500 = tableInfo(counter).y(1:floor(500/(1000/341)));
...
if reprocess == 1
    ...
    for ii = 1:length(unique([tableInfo(:).Subject]))
        
        idx = find([tableInfo(:).Subject] == ii & ...
            [tableInfo(:).Performance] < 3);
        
        [thresh(ii),~,~,xi{ii},yi{ii}] = psyfitCrowding(...
            round([tableInfo(idx).Size]*2,1), ... %rounding to reflect monitor resolution
            [tableInfo(idx).Performance], 'DistType', 'Normal',...
            'Xlim', xl, 'Ylim', yl,...
            'Chance', gamma, 'Extra');
        
    end
    
    snellen2020 = thresh*20;
...
for ii = 1:length(unique([tableInfo(:).Subject]))
    tableSubjectInfo(ii).ID = ii;
    tableSubjectInfo(ii).Stroke = ~control(ii);
    tableSubjectInfo(ii).Sex = sexAll(ii);
    tableSubjectInfo(ii).Age = ageAll(ii);
    tableSubjectInfo(ii).Acuity = snellen2020(ii);
    tableSubjectInfo(ii).WeeksPostLesion = weeksPostLes(ii);
    tableSubjectInfo(ii).TextHemifield = affHemText{ii};
    if ~control(ii)
        tableSubjectInfo(ii).AngleOfLoss = ...
            vfResults.patientInfo.(huxID{ii}).middleAngleLoss;
        tableSubjectInfo(ii).VPMap = vfResults.patientInfo.(huxID{ii}).matrix{1};
    end
    
    idxTask = find([tableInfo(:).Subject] == ii & ...
        [tableInfo(:).Performance] < 3 & ...
        [tableInfo(:).bcea] < 6*60);
    tableSubjectInfo(ii).bceaAllTask = get_bcea([tableInfo(idxTask).x]/60,...
        [tableInfo(idxTask).y]/60);
    
    idxFixation = find([tableInfo(:).Subject] == ii & ...
        [tableInfo(:).Performance] == 3);
    
    %     if ii == 3 || ii == 24
    %         idxFixation = find([tableInfo(:).Subject] == ii & ...
    %             [tableInfo(:).Performance] == 3 & ...
    %             [tableInfo(:).bcea] < 10000);
    %     else
    %         idxFixation = find([tableInfo(:).Subject] == ii & ...
    %             [tableInfo(:).Performance] == 3 & ...
    %             [tableInfo(:).bcea] < 5000);
    %     end
    tableSubjectInfo(ii).bceaAllFix = get_bcea([tableInfo(idxFixation).x]/60,...
        [tableInfo(idxFixation).y]/60);
    
    tempX = [tableInfo(idxFixation).x];
    tempY = [tableInfo(idxFixation).y];
    [tempX, id] = unique(tempX);
    tempY = (tempY(id'));
    fixIdes = find(tempX < 60 & tempX > -60 & tempY < 60 & tempY > -60);
    tableSubjectInfo(ii).fixX = tempX(fixIdes);
    tableSubjectInfo(ii).fixY = tempY(fixIdes);
...
for ii = (find([tableSubjectInfo(:).Stroke] == 1)) %Patients
    ...
    if counter == 1
        heatMapIm = generateHeatMapSimple( ...
            [tableSubjectInfo(ii).fixX], ...
            [tableSubjectInfo(ii).fixY], ...s
            'Bins', 30,...
            'StimulusSize', 0,...
            'AxisValue', axisVal,...
            'Uncrowded', 4,...
...
for ii = (find([tableSubjectInfo(:).Stroke] == 0)) %Controls
    subplot(4,4,counter)
    if counter == 1
        heatMapIm2{ii} = generateHeatMapSimple( ...
            [tableSubjectInfo(ii).fixX], ...
            [tableSubjectInfo(ii).fixY], ...s
            'Bins', 30,...
            'StimulusSize', 0,...
...
figure;
boxplot([tableSubjectInfo.bceaAllFix],...
...
[h,p,~,sts] = ttest2([tableSubjectInfo(~control).bceaAllFix],...
    [tableSubjectInfo(controls).bceaAllFix]);
...
imshow(im30)
counter = 9;
for ii = 1:length(tableSubjectInfo)
    if tableSubjectInfo(ii).Stroke == 0
        continue;
    end
    ...
    subplot(6,4,counter)
    temp = tableSubjectInfo(ii).VPMap;
    temp(isnan(temp)) = -20;
...
for ii = 1:length(tableSubjectInfo)
    if tableSubjectInfo(ii).Stroke == 0
        tableSubjectInfo(ii).allRotated = ([tableSubjectInfo(ii).fixX;...
            tableSubjectInfo(ii).fixY]);
        continue;
    end
    lossIdx = find(vfResults.patientInfo.(huxID{ii}).deficit);
    ...
    v = [tableSubjectInfo(ii).fixX;tableSubjectInfo(ii).fixY]';
    theta = -rotateAngle;
    R = [cosd(theta) -sind(theta); sind(theta) cosd(theta)];
    vRTemp = (v*R);
    tableSubjectInfo(ii).allRotated = [vRTemp(:,1)';...
        vRTemp(:,2)'];
...
forleg(1) = plot(median(patientsAllX),median(patientsAllY),...
...
title('Controls')
ellipseXY(patientsAllX, patientsAllY, 65, 'r' ,2)
plot(median(patientsAllX),median(patientsAllY),'d','Color','w','MarkerFaceColor','k')
%% Figure3B and C
for ii = 1:length(tableSubjectInfo)
    rotForHistogram{ii} = tableSubjectInfo(ii).allRotated;
end

[pY_FullFixation, meanValsCL, meanValsPT, meansValsIndY] = ...
    histogramNormalizedForRotation(1:26, ~[tableSubjectInfo(:).Stroke], ...
    rotForHistogram,...
    rotForHistogram, 2, 0); %x = 1 or y = 2
...
function [traces, counter, valid, em] = countingTrials(pptrials)%,numTooShort,numBlinks,numNoTracks,numSaccades,numMicroSaccades)
...
traces = [];
paramsCT.sampling = 341;
paramsCT.spanMax = 200;
paramsCT.spanMin = 2;
...
for ii = 1:length(pptrials)
    ...
    if  ceil(pptrials{ii}.TimeTargetON) > 0 %% TASK TRIAL
        ...
        timeOn = round(pptrials{ii}.TimeTargetON/(1000/paramsCT.sampling));
        timeOff = round(min(pptrials{ii}.TimeTargetOFF/(1000/paramsCT.sampling)-1, ...
            pptrials{ii}.ResponseTime/(1000/paramsCT.sampling)));
        if timeOn + floor(timeOff) > length(pptrials{ii}.x.position)
            continue;
        end
        x = pptrials{ii}.x.position(ceil(timeOn):floor(timeOff)) + ...
            pptrials{ii}.xoffset * pptrials{ii}.pixelAngle;
        y = pptrials{ii}.y.position(ceil(timeOn):floor(timeOff)) + ...
            pptrials{ii}.yoffset * pptrials{ii}.pixelAngle;
        
        em.xShift{ii} = x - single(pptrials{ii}.TargetEccentricity * pptrials{ii}.pixelAngle);
        em.targetEcc(ii) = single(pptrials{ii}.TargetEccentricity * pptrials{ii}.pixelAngle);
        em.x{ii} = x;
        em.y{ii} = y;
        em.eccTarget(ii) =  single(pptrials{ii}.TargetEccentricity * pptrials{ii}.pixelAngle);
        ...
        em.actualEccTargetDist(ii) = double( mean(x)...
            - single(pptrials{ii}.TargetEccentricity * pptrials{ii}.pixelAngle));
        span = em.span(ii);
        %Performance
        em.performance(ii) = pptrials{ii}.Correct;
        em.size(ii) = pptrials{ii}.pixelAngle * double(pptrials{ii}.TargetStrokewidth);
        em.prl(ii) = mean(hypot(x,y));
    ...
    elseif ceil(pptrials{ii}.TimeFixationON) > 0
        ...
        x = pptrials{ii}.x.position((ceil(timeOn/(1000/paramsCT.sampling))):timeOff) + pptrials{ii}.pixelAngle * pptrials{ii}.xoffset;
        y = pptrials{ii}.y.position((ceil(timeOn/(1000/paramsCT.sampling))):timeOff) + pptrials{ii}.pixelAngle * pptrials{ii}.yoffset;
        
        em.span(ii) = quantile(sqrt((x-mean(x)).^2 + (y-mean(y)).^2), .95);
        em.meanXSpan(ii) = (mean(x)); %%mean span from 0, essentially causing target to be a specific distance
        em.actualEccTargetDist(ii) = double( mean(x)...
            - single(pptrials{ii}.TargetEccentricity * pptrials{ii}.pixelAngle)); %%Take the DIFFERENCE between the 2
        span = em.span(ii);
        em.prl(ii) = mean(hypot(x,y));
    ...
    huxOffset = 180;
    if sum(abs(x) > huxOffset) > 3 || ...
            sum(abs(y) > huxOffset) > 3
    ...
    %% Checks for BLINKS
    if find(pptrials{ii}.blinks.start > timeOn & ...
            pptrials{ii}.blinks.start < timeOff)...
    ...
    %% Make sure correct eccentricity
    if ~(round(double(pptrials{ii}.TargetEccentricity)*pptrials{ii}.pixelAngle)) ...
            == 0%%%Wrong Ecc
        valid.wrongecc(ii) = true;
        counter.wrongecc = counter.wrongecc+1;
        output(ii) = 1;
        continue;
    end
    ...
    if span < paramsCT.spanMin || span > paramsCT.spanMax %%%IF Span is too large/too small
        valid.span(ii) = true;
    ...
    if ~isfield(params,'OnlyTemp')
        ...
        %         fileLocation = sprintf('../../Data/%s/Graphs/TossedInformation.xlsx',params.subject);
        %         if figures.HUX_RUN == 1
        %             writetable(data,fileLocation);
        %         end
    end
end
    function [spanV1, ...
            ...
            prlDistance,...
            prlDistanceX, ...
            prlDistanceY] = getDriftChar(x, y, smoothing, cutseg, maxSpeed, Fs)
        % smoothing: 41
        ...
        instSpX = smx1*Fs; instSpY = smy1*Fs;%transformed to acrmin/sec
